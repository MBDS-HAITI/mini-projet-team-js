      ...
      <Grid container alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        ...
        <Grid item>
          <Stack direction="row" spacing={1}>
            <Button variant="outlined" startIcon={<PrintIcon />} component={RouterLink} to="/print/users">Aperçu liste</Button>
            <Button variant="contained" startIcon={<AddIcon />} component={RouterLink} to="/register">Créer un utilisateur</Button>
          </Stack>
        </Grid>
      </Grid>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell><b>Bloqué</b></TableCell>
          ...
          <TableBody>
            {loading ? (
              ...
              filtered.map((u) => {
                ...
                <TableCell>{u.blocked ? "Oui" : "Non"}</TableCell>

                    <TableCell align="right">
                      <Stack direction="row" spacing={1} justifyContent="flex-end">
                        <Button size="small" variant="outlined" startIcon={<VisibilityIcon />} onClick={() => navigate(`/users/${u._id}`)}>
                          Détails
                        </Button>

                        <Button size="small" variant="outlined" startIcon={<EditIcon />} onClick={() => openEdit(u)}>
                          Modifier
                        </Button>

                        <Button
                          size="small"
                          variant={u.blocked ? "contained" : "outlined"}
                          color={u.blocked ? "success" : "error"}
                          startIcon={u.blocked ? <LockOpenIcon/> : <BlockIcon/>}
                          onClick={async () => {
                            try {
                              if (u.blocked) await api.post(`/api/users/${u._id}/unblock`);
                              else await api.post(`/api/users/${u._id}/block`);
                              await loadAll();
