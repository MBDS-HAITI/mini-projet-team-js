...
export default function GradeEdit() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError("");
      try {
        const [gr, st, co] = await Promise.all([
        ...
        const g = gr.data || {};
        setForm({
          studentId: g.student?._id || g.studentId || "",
          courseId: g.course?._id || g.courseId || "",
          note: g.note ?? 0,
          sur: g.sur ?? 100,
          periode: g.periode ?? "Semestre 1",
          remarque: g.remarque ?? "",
        });
        ...
        setError(
          e?.response?.data?.message ||
            "Erreur chargement des données pour la note"
        );
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        e?.response?.data?.message || "Erreur lors de la modification de la note"
      );
    } finally {
      setSaving(false);
    }
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 900, mx: "auto" }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Mettre à jour la note d'un étudiant pour un cours.
        </Typography>
      </Box>
